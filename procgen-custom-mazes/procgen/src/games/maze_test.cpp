...
const std::string NAME = "maze_test";

const float REWARD = 10.0;
const float REWARDVARIANT = -10.0;
// const float REWARDVARIANT2 = -20.0;

const int GOAL = 2;
const int GOALVARIANT = 3;
// const int GOALVARIANT2 = 4;

class MazeTest : public BasicAbstractGame {
  public:
    std::shared_ptr<MazeGen> maze_gen_test;
    int maze_dim = 0;
    int world_dim = 0;
    ...
    void game_reset() override {
        ...
        maze_gen_test->generate_maze();
        // maze_gen_aisc-> deterministic_place(GOAL, false);
        // maze_gen_aisc-> deterministic_place(ARROW, true);
        maze_gen_test->place_objects(GOAL, 1);
        maze_gen_test->place_objects(GOALVARIANT,1);
        // maze_gen_test->place_objects(GOALVARIANT2,1);
        ...
        for (int i = 0; i < maze_dim; i++) {
            for (int j = 0; j < maze_dim; j++) {
                int type = maze_gen_test->grid.get(i + MAZE_OFFSET, j + MAZE_OFFSET);
    ...
    void game_step() override {
        ...
        if (get_obj(ix, iy) == GOALVARIANT) {
            set_obj(ix, iy, SPACE);
            step_data.reward += REWARDVARIANT;
            step_data.level_complete = true;
        }

        // if (get_obj(ix, iy) == GOALVARIANT2) {
        //     set_obj(ix, iy, SPACE);
        //     step_data.reward += REWARDVARIANT2;
        //     step_data.level_complete = true;
        // }
