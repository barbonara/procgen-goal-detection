...
class MazeGameGem : public BasicAbstractGame {
  public:
    std::shared_ptr<MazeGen> maze_gen_gem;
    int maze_dim = 0;
    int world_dim = 0;
    ...
    void game_reset() override {
        ...
        maze_gen_gem->generate_maze();
        // maze_gen_aisc-> deterministic_place(GOAL, false);
        // maze_gen_aisc-> deterministic_place(ARROW, true);
        maze_gen_gem->place_objects(GOAL, 1);
        maze_gen_gem->place_objects(GOALVARIANT,1);
        ...
        for (int i = 0; i < maze_dim; i++) {
            for (int j = 0; j < maze_dim; j++) {
                int type = maze_gen_gem->grid.get(i + MAZE_OFFSET, j + MAZE_OFFSET);
...
REGISTER_GAME(NAME, MazeGameGem);
